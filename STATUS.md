...
- **Play internal (track, not Internal app sharing):** owner Gmail allowlisted 2026-08-15. Release is on the internal testing **track** (0.1.0 / versionCode 2, status `completed` — reconfirmed 2026-08-16 via Play Developer API). Play does **not** email testers — add each friend's Play Store Gmail to the email list, then send the same opt-in link (Play Console → Testing → Internal testing → Testers). The “Anyone you shared the link with can download” toggle is **Internal app sharing**, a different product — do not use it for friends (hidden Play Store setting, 60-day links, re-signed cert / FCM, separate upload from `eas submit`). 0 friends invited so far.
- **2026-08-16 — Play Store “Item not found” (sleeping satellite):** a friend hit this while trying to install. The build is on the track; the public listing is not. `https://play.google.com/store/apps/details?id=com.rkilani.events` 404s for anyone who has not opted in, and Play search will not find **Events** / **Shared Events** until a production listing exists. Recovery: add the Gmail the phone’s Play Store is signed into → send the Console opt-in link (`https://play.google.com/apps/testing/com.rkilani.events` or the copy from Testers) → they tap **Become a tester** on that account → then install. Do not send the details URL. Do not rebuild.
