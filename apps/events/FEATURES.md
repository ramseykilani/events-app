...
| [US Phone Numbers](#us-phone-numbers) | Implemented | A2P campaign **VERIFIED** 2026-09-17 — the US `30034` carrier block is lifted; a real US sign-in OTP delivered 2026-09-15 — `docs/a2p-registration.md`. |
...
**Status:** Implemented (2026-08-17) — config, not code. Found in the 2026-08-15 owner device smoke (`manual-tests/manual_test_report_2026-08-15-device.md`). Shipped via Management API auth-config PATCH. Current template (updated 2026-09-05 for A2P sample-1 sync): `sms_template = "Shared Events: {{ .Code }} is your sign-in code. Reply STOP to opt out."` (~69 chars with a 6-digit code, single GSM-7 segment, names the registered brand, STOP on the OTP). No app code changes; no EAS rebuild.
...
**Status:** Implemented 2026-09-17 — the fix was A2P registration, not app logic. Campaign `QE2c6890da8086d771620e9b13fadeba0b` is **VERIFIED** (owner report, API-confirmed; approved after resubmission #5 with the hosted `/opt-in` evidence page — the four rejections are the dated log in Open Questions). The US `30034` block is lifted: the last `30034`s are 2026-09-11 (pre-approval) and a real US sign-in OTP delivered 2026-09-15. State + playbook: `docs/a2p-registration.md` and `STATUS.md` → A2P 10DLC registration. Recorded 2026-08-16 from internal testing.
...
- [x] A real US number can be added and shared to (shares to a real US number were created 2026-09-11 — the mechanic never depended on registration; only the SMS leg was blocked)
- [ ] Share SMS to that number is delivered (or a visible failure — see [Share Delivery Status](#share-delivery-status)) — no US-bound share text has gone out since approval; the first one confirms (the block was account-wide, and the OTP leg already delivers to a US handset)
- [x] Sign-in with a US number works when the number is a real user, not only a 555 test OTP (real US OTP delivered 2026-09-15, observed in the Twilio delivery scan)
...
- 2026-08-19: sole-proprietor brand approved + verified (2026-08-17); campaign `QE2c6890da8086d771620e9b13fadeba0b` submitted and **rejected same day** — TCR error 30909, `MESSAGE_FLOW`: the opt-in CTA (the app sign-in screen) couldn't be verified because it never explicitly says the user is agreeing to receive texts.
- 2026-09-04: consent line live on production; owner resubmitted the same campaign SID (message_flow quotes the on-screen line; sample 2 refreshed). Status went **IN_PROGRESS**, then **FAILED** 2026-09-05 (`30908` + `30893`). Full state and playbook: `docs/a2p-registration.md`; live table: `STATUS.md` → A2P 10DLC registration.
- 2026-09-05: privacy page now carries the TCR non-sharing clause and names Shared Events / Ramsey Kilani; sample 1 + Auth `sms_template` are `Shared Events: [code] is your sign-in code. Reply STOP to opt out.` Same SID resubmitted; live status **IN_PROGRESS**.
- 2026-09-05 (later): **FAILED** a third time — same `30908` + `30893`. API inspection found the real causes: the registered brand name is `Ramsey Kilani` (sole-prop EndUser `brand_name`), which sample 1 never contained, and the campaign had no dedicated `PrivacyPolicyUrl`/`TermsAndConditionsUrl` (required since 2026-06-30; inline `message_flow` links don't count). Resubmission #4 by API: Auth template + sample 1 now `Shared Events by Ramsey Kilani: …`, both policy URLs sent, same SID, status **IN_PROGRESS** (`PENDING_DCA1_REVIEW`). Sign-in legal links switched to extensionless URLs (staging).
- 2026-09-10: **FAILED** a fourth time — `30909` returned (CTA evidence); `30908`/`30893` stayed cleared. The sign-in screen is a client-rendered JS app, so a non-browser review tool sees an empty shell. Fix: server-rendered evidence page `public/opt-in.html` (+ hosted sign-in screenshot) deployed to production (`/opt-in`), `message_flow` points reviewers at it; same SID resubmitted by API, status **IN_PROGRESS**. Playbook: `docs/a2p-registration.md`.
- 2026-09-17: campaign **VERIFIED** — registration complete (owner report, API-confirmed: `campaign_status: VERIFIED`, errors empty). Delivery scan: the last `30034`s are 2026-09-11 (pre-approval); a real US sign-in OTP delivered 2026-09-15. US block lifted for both SMS legs.
...
- [x] People header shows a gear; the footer (Notifications / Your name / Sign out / Delete account) is gone. Owner 2026-09-02: the gear sits too close to Add — [KI-017](manual-tests/known_issues.md), not a blocker.
...
- **Phase 1 — event detail** (`app/(app)/event/[id].tsx`): AppHeader, tiered action stack, one content measure. (The location-row restyle to `linkText` already landed with the UX-13 ruling — a visual no-op under the merged values.) Plus the two grammar-D modal headers (PeoplePicker, ManualAddPersonModal) — the worst targets in the app.
...
- **Remaining audit minors fold in per the whole-app coherence ruling (2026-09-01):** the privacy-link token question (UX-18) and the destructive-verb vocabulary (UX-22) land with Phase 3's cleanup; the receipt-page dialect (UX-25) stays a separate follow-up sweep — `receipt/` is its own Pages deploy, not app code.
...
- **Heard-from field (2026-09-05).** The form asks "How did you hear about Events?" (required free text, 200-char cap). The value lands on `beta_signups.heard_from` and on a second line of the owner SMS. See [Beta Signup Heard-From](#beta-signup-heard-from).
...
- [x] `beta_signups` RLS covered by SQL tests (22 assertions); fast checks green; STATUS.md tester section updated
...
One static page — "Events" — in the app's design language, on its own Cloudflare Pages project (receipt/ precedent). Copy adapted from `docs/events-product.md`: what it is (a calendar of events your people share with you), what it isn't (no feed, no likes, nothing to scroll), and how to get in during beta. The page carries the app's theme swatch (owner addition 2026-09-02): the same two-tone disc as the calendar header, previewing the other mood, cycling Paper and Evening — Paper by default, persisted across visits, OS color scheme never read. The only CTA is a `mailto:` to the owner with a prefilled template containing everything fulfillment needs:
...
- Both palettes (Paper and Evening) ported from current `constants/Colors.ts` values as CSS variables — not from `receipt/index.html`, whose accent predates the 2026-09-01 contrast fixes. Typeface voice follows the mood per design doc §4 (serif titles in Paper, clean sans in Evening). The Paper E’s monogram from `assets/` heads the page and serves as favicon.
- The theme swatch behaves exactly as in the app (design doc §7): a small circular two-tone disc in the *other* mood's colors, the destination named in the accessibility label ("Switch to Evening theme"), never a sun/moon icon. Browser chrome follows the mood — `theme-color`, `color-scheme`, page background (the static-page equivalent of `applyWebBrowserChrome`). A tiny inline `<head>` script applies the saved mood before first paint, so a returning Evening visitor never sees a Paper flash.
- The page should read as the same object as the app, not a marketing cousin (owner clarification 2026-09-02): same token values, same type voices and scale (§4), same radius/depth spectrum (§5), same nothing-moves motion rule (§6), same accent discipline (§3 — on the page the accent is spent on the swatch, links, and little else; the mailto CTA wears the primary-button roles). It is a hand-port to static HTML/CSS — the receipt page's drift (stale accent, out-of-spectrum radii, sans-only type; UX-25) is the cautionary example, not the template.
...
- **Verify bar (owner question 2026-09-02):** the repo fast checks gate the staging push as always, and the page gets its own Playwright spec in the e2e suite — `e2e/receipt.spec.ts` is the pattern (static dir served locally, the live deployment never touched): swatch toggle + persistence + no first-paint flash, mailto href and copyable fallback, both moods at desktop and ~390px, `noindex` meta, no web-app or custom-scheme links. App-only layers don't apply (no Jest/RNTL, SQL, or RN conventions lint on static HTML — the port-from-`Colors.ts` discipline is the token rule's equivalent). The app's ship-it protocol does NOT govern this page: it deploys on its own lever to its own Pages project and never touches the app (receipt precedent). First-deploy gate instead: owner approves final copy, spec green, one agent click-through of the live preview (both moods, desktop + mobile) before the URL goes out. The launch-page transition (indexed, store badges, deep links) gets a real review pass as part of the launch checklist.
- **Deploy protocol (owner question 2026-09-02):** agent-run, no CI automation — `npm run deploy:landing` = `wrangler pages deploy landing --project-name=<chosen> --branch=main` (the first deploy creates the project; same `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` secrets as `deploy:receipt`). Deploy only when the staging push is green. A `--branch=preview` deploy gives a preview URL for review without touching the production URL. The app's staging→production promotions and CI web deploys never touch this project, and app deploys never redeploy the page.
...
The production landing page (`landing/` → https://events-landing.pages.dev) is a single centered column: headline, sub, mock calendar, beta block. The Three-One-Four exercise produced a stronger hero arrangement and a principles section that says what the product is better than the current page does. Those pieces should move to the production page; the seed-specific artifacts should not.
...
- The 01/02/04 numbering and missing-03 footnote (renumber to 01/02/03).
- The "The seed" colophon band and the raw seed string — the exercise's story, not product communication.
- The raspberry hue-314 palette and blush ground — production stays on the role tokens ported from `constants/Colors.ts` ("same object as the app"). A landing page in its own register is a separate owner decision.
- Zero-JavaScript — a nice property of the candidate, but the theme swatch (owner feature) needs JS; not a goal.
...
- Desktop polish (owner-approved 2026-09-03, after the production deploy of the pull-ins): the mock grows to 480px so the product shot carries the hero's right column, every section spans the hero's 1080px measure (owner: the narrower below-hero sections read ragged against the wide hero; prose inside keeps a 65ch reading measure), and the hero takes the candidate's 56/72px vertical rhythm. Mobile is untouched — phone viewports never reach these caps.
...
**Status:** Planned — polish, not a blocker. Recorded 2026-09-11 from owner feedback after creating an event. **Not designed; do not implement from this section.**
