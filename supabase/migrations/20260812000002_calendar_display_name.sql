-- Display-name attribution: when the recipient has no contact row for the
-- sharer (or one without a contact_name), "From X" falls back to the sharer's
-- own display_name. The recipient's own label always wins when present.
--
-- Privacy: display_name is only ever exposed here to recipients of an actual
-- share (incoming CTE), consistent with send-notification's attribution.
