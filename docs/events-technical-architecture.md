...
**Tap handler (push):** Configured in `app/_layout.tsx` — tapping a notification is supposed to navigate to `/(app)/event/[id]` with the recipient's own row id (Who's Coming asker pushes carry the asker's row id). The detail screen resolves ids it doesn't own via a fallback (`events WHERE from_event_id = :id` — the caller's copy of a followed sender's row) before showing "Event not found" / access-removed. **Known miss ([KI-018](../manual-tests/known_issues.md)):** a tap from a killed app currently lands on the calendar home instead — the handler never reads the last notification response after boot, and a `router.push` while the boot spinner is up is lost to the session `replace('/(app)')`.
