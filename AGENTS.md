...
The beta landing page (`landing/` → FEATURES.md → Beta Landing Page) is a one-page static site on its own Cloudflare Pages project, same shape as `receipt/`. Deploy is agent-run, no CI automation: `npm run deploy:landing` (`wrangler pages deploy landing --project-name=<chosen> --branch=main`; the first deploy creates the project, same Cloudflare secrets as `deploy:receipt`). Deploy only when the staging push is green; `--branch=preview` gives a review URL without touching production. The app's staging→production promotions and CI web deploys never touch this project. First-deploy gate: owner approves final copy, the e2e spec is green, and one agent click-through of the live preview (both moods, desktop + mobile) has happened before the URL goes out.
