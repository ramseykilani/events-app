...
- **Sign-in discipline:** sign in once per run and reuse stored sessions; never sign in per test. Before running e2e locally, provision your own pair — `node scripts/create-test-accounts.mjs --fresh-pair`, then export the printed `E2E_PHONE_A/B`. Password sign-in (see Signing in) fires no SMS. OTP on a registered test number uses `sms_test_otp` and does not call Twilio.
