...
The beta landing page (`landing/` → FEATURES.md → Beta Landing Page) is a one-page static site on its own Cloudflare Pages project, same shape as `receipt/`. Deploy is agent-run, no CI automation: `npm run deploy:landing` (`wrangler pages deploy landing --project-name=events-landing --branch=main`, same Cloudflare secrets as `deploy:receipt`). The `events-landing` project was created 2026-09-02 via `wrangler pages project create events-landing --production-branch=main` — wrangler 4.120 no longer auto-creates a Pages project on first deploy, despite the receipt-era note in FEATURES.md. Live at https://events-landing.pages.dev. Deploy only when the staging push is green; `--branch=preview` gives a review URL without touching production. The app's staging→production promotions and CI web deploys never touch this project. First-deploy gate: owner approves final copy, the e2e spec is green, and one agent click-through of the live preview (both moods, desktop + mobile) has happened before the URL goes out. The Three-One-Four redesign candidate (`landing-v2/` → FEATURES.md → Landing Page Redesign (Three-One-Four)) is a separate static site on its own project `events-landing-v2` (created 2026-09-03), live at https://events-landing-v2.pages.dev via `npm run deploy:landing-v2` — same deploy rules, and it never replaces `landing/` without an owner call.

Notification SMS carries no app/web links (decision 2026-08-09 — see `docs/distribution-strategy.md`); the one exception is the internal-testing signup invite on the non-app variant, which points at the self-serve beta form (`https://events-landing.pages.dev/signup` — FEATURES.md → Beta Signup Pipeline; owner approved the link 2026-09-03, replacing the email CTA). At launch, store links (non-users) and an event deep link (app users) return together — spec: `FEATURES.md` → SMS Links at Launch; do not implement one without the other, and not before listings exist. `send-notification` reads only the Twilio secrets today; `WEB_APP_URL` is set but unused (event-link base at launch). The placeholder `IOS_APP_STORE_URL` secret was removed 2026-08-09 (restored at launch for the non-app CTA). Build the static bundle with `npm run build:web` (→ `dist/`).
...
- EAS project environment variables `EXPO_PUBLIC_SUPABASE_URL` / `EXPO_PUBLIC_SUPABASE_ANON_KEY` (plaintext; development + preview + production) — created 2026-08-15. EAS builds do not read local `.env`; without these the bundle has no Supabase config. (Pre-hardening this was an instant launch crash — `createClient` threw at module scope; `packages/infra/supabase.ts` now falls back to a placeholder, but the app is useless without real values.) Inspect with `eas env:list --environment preview`; recreate per SETUP.md → EAS Builds.
...
- Sign-in surfaces SMS send failures to the user via a short `showAlert` (expected auth mistakes like `sms_send_failed`, rate limits, and expired OTP use `getAuthUserMessage` from `@family/infra`). Unexpected failures still use `showError` (detailed alert with code/details). Check that dialog when auth testing fails.
- `Alert.alert` is a no-op on web. Use `showAlert`/`showConfirm` from `@family/infra` (or `showError` for errors) for any user-facing dialog so it renders as `window.alert`/`window.confirm` in the browser.
