...
| [US Phone Numbers](#us-phone-numbers) | Planned | Diagnosed: unregistered 10DLC (`30034`). A2P campaign **IN_PROGRESS** (resubmission #5, 2026-09-10: hosted opt-in evidence page for the JS-app CTA) — `docs/a2p-registration.md`. |
...
**Status:** Planned — diagnosed 2026-08-17 (unregistered 10DLC; see first Open Questions bullet). The fix is A2P registration, not app logic: brand approved + verified 2026-08-17; campaign rejected 2026-08-19 on CTA evidence (TCR 30909); consent-line fix shipped and resubmitted 2026-09-04; **FAILED** again 2026-09-05 on `30908` (privacy clause) + `30893` (sample 1) — `30909` did not return; fixed and resubmitted, then **FAILED** a third time hours later on the same two codes. Root causes of rejection #3: sample 1 named the app, not the registered brand (**`Ramsey Kilani`**), and the campaign lacked the dedicated `PrivacyPolicyUrl`/`TermsAndConditionsUrl` fields (required since 2026-06-30). Resubmission #4 (2026-09-05, all API): Auth template + sample 1 are now `Shared Events by Ramsey Kilani: …`, policy URLs sent. Rejection #4 (2026-09-10) returned `30909` (CTA evidence): the sign-in screen is a client-rendered JS app, so non-browser review tools see an empty shell. Resubmission #5 (2026-09-10): server-rendered evidence page live at https://shared-events.pages.dev/opt-in (verbatim consent line + hosted sign-in screenshot) and referenced from `message_flow`; campaign `QE2c6890da8086d771620e9b13fadeba0b` is **IN_PROGRESS**. US-bound SMS stays blocked (`30034`) until approval. State + playbook: `docs/a2p-registration.md` and `STATUS.md` → A2P 10DLC registration. Recorded 2026-08-16 from internal testing.
