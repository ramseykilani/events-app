...
3. For development, you can use the built-in Supabase test OTP. Under **Authentication > Settings**, scroll to "Test Users" or "Test OTP" and add a phone number / OTP pair (e.g. `+15555550100` / a 6-digit code of your choosing — this repo's shared value lives in the `E2E_TEST_OTP` secret, never in the tree). Auth then returns that code without calling Twilio (`message_id: test-otp`). Do not use `/otp` to *create* those users — `scripts/create-test-accounts.mjs` registers `sms_test_otp` first, then creates them via Auth Admin with `phone_confirm: true`. Share SMS is a separate path: `send-notification` skips NANP **area code** 555 (a 416-555 placeholder still sends). Full test-account rules: AGENTS.md → Signing in (test accounts).
...
5. Set the phone-auth SMS template (Authentication → Sign In / Up → SMS template, or Management API `sms_template`) to `Shared Events: {{ .Code }} is your sign-in code. Reply STOP to opt out.` so the verification text names the registered brand and matches the A2P sample. Keep it to one GSM-7 segment.
...
| New architecture (`newArchEnabled`) | **Enabled since 2026-09-01** (owner device-verified on Android). Hard requirement: `react-native-screens` must stay ≥4.17.1 — earlier 4.x (incl. the SDK 54 default pin `~4.16.0`) has the [`ScreenStack.getChildDrawingOrder()` off-by-one](https://github.com/software-mansion/react-native-screens/issues/3096) that instantly crashed Android new-arch builds. The repo carries `~4.23.0` via `expo.install.exclude` (4.24 breaks expo-router; 4.25+ needs RN 0.82). If screens is ever downgraded, flip the flag back to `false` in `app.config.js` and rebuild. | Keep the screens floor; the flag lives in `app.config.js` |
